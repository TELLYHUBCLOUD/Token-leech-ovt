...
class TgUploader:
    ...
    async def _final_message(self, ss_image, media_info: bool=False):
        ...
        # Video Stream Mod Base URL logic
        if getattr(self._listener, 'vidMode', None) and self._listener.vidMode[0] == 'vid_stream' and self._listener.vidMode[2].get('is_stream', True):
            video_url = stream_dl_links[1] or stream_dl_links[0]
            if video_url:
                base_url = config_dict.get('BASE_URL', '').rstrip('/')
                if not base_url:
                    LOGGER.error("BASE_URL not set! Please set your Video Stream Mod URL in config.")
                else:
                    try:
                        import json
                        import base64
                        ...
                        self._buttons.button_link('⬇ Download', await sync_to_async(short_url, stream_dl, self._listener.user_id), 'header')
