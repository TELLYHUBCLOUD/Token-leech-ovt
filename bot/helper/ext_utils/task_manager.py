...
async def check_limits_size(listener, size, playlist=False, play_count=False):
    ...
    if config_dict['PREMIUM_MODE'] and not is_premium_user(listener.user_id, getattr(listener.message, 'chat', getattr(listener, 'message')).id if getattr(listener, 'message', None) else None):
        mdl = float(config_dict['NONPREMIUM_LIMIT']) if config_dict['NONPREMIUM_LIMIT'] else 0.0
        torddl = zuzdl = leechdl = mdl
        megadl = min(float(megadl), mdl) if megadl else mdl
        max_pyt = 10
