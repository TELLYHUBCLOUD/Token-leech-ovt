...
async def get_user_settings(from_user, data: str, uset_data: str):
    ...
    if not data:
        ...
        thumb_status = 'Activated' if thumbmsg == ' ✓' else 'None'
        meta_status = 'Activated' if metadata == ' ✓' else 'Original'
        asgroup_status = 'Activated' if mediagroup == ' ✓' else 'Deactivated'
        userdump_status = 'Activated' if dumpch != ' ✘' else 'Deactivated'
        gdrive_status = 'Activated' if gdxmsg == ' ✓' else 'Deactivated'
        session_status = 'Activated' if sesmsg == ' ✓' else 'None'
        ytdlp_status = 'Activated' if yto != ' ✘' else 'None'
        customcap_status = 'Activated' if custom_cap == ' ✓' else 'Deactivated'
        pre_status = 'Activated' if premsg != ' ✘' else 'None'
        suf_status = 'Activated' if sufmsg != ' ✘' else 'None'
        rem_status = 'Activated' if rmmsg != ' ✘' else 'None'
        split_size = get_readable_file_size(config_dict['LEECH_SPLIT_SIZE'])

        text = ('〔 𝖲𝖸𝖲𝖳𝖤𝖬  𝖢𝖮𝖭𝖳𝖱𝖮𝖫  𝖯𝖠𝖭𝖤𝖫 〕\n\n'
                f'➻ USER : {from_user.mention}\n\n'
                f'------『 𝖲𝖤𝖳𝖳𝖨𝖭𝖦𝖲 』------\n\n'
                f'● Leech Type: {ltype.upper()}\n'
                f'● Default Upload: {du.upper()}\n'
                f'● Split Size: {split_size}\n'
                f'● RClone: {rccmsg.strip()} {rclone_status}\n\n'
                f'------『 𝖢𝖮𝖭𝖥𝖨𝖦𝖴𝖱𝖠𝖳𝖨𝖮𝖭 』-----\n\n'
                f'» DM Mode: {sendpm.strip()} {dmmode_status}\n'
                f'» SS Mode: {sendss.strip()} {ssmode_status}\n'
                f'» Thumbnail: {thumbmsg.strip()} {thumb_status}\n'
                f'» Metadata: {metadata.strip()} {meta_status}\n'
                f'» As Group: {mediagroup.strip()} {asgroup_status}\n'
                f'» UserDump/Tds: {dumpch.strip()} {userdump_status}\n'
                f'» Custom GDrive: {gdxmsg.strip()} {gdrive_status}\n'
                f'» Session String: {sesmsg.strip()} {session_status}\n'
                f'» YT-DLP Options: {yto.strip()} {ytdlp_status}\n\n'
                f'------『 𝖢𝖴𝖲𝖳𝖮𝖬  𝖳𝖤𝖷𝖳 』------\n\n'
                f'» Caption: {capmode.upper()}\n'
                f'» Custom Caption: {custom_cap.strip()} {customcap_status}\n'
                f'» Prename: {premsg.strip()} {pre_status}\n'
                f'» Sufname: {sufmsg.strip()} {suf_status}\n'
                f'» Remname: {rmmsg.strip()} {rem_status}\n\n'
                f'-----------------------------\n')

        if status_user or premium_left or daily_limit:
            text += '\n' + status_user + premium_left + daily_limit
